...
export default function ServiceCard({ service, href, ariaLabel }) {
    ...
    >
      ...
      <div className="relative w-20 h-20 mb-5">
        ...
        <div className="absolute inset-2 rounded-full bg-[#ECEFFF] flex items-center justify-center">
          <Image src={service.icon} alt="service icon" width={36} height={36} className="w-9 h-9" />
        </div>
      </div>

      {/* Title */}
      <h3 className="text-black font-bold text-lg sm:text-xl mb-3">{service.title}</h3>
  ...
  if (href) {
    return (
      <Link href={href} aria-label={ariaLabel || service.title} className="block focus:outline-none focus:ring-2 focus:ring-[#236BFD] rounded-[12px]">
        {CardInner}
      </Link>
    );
  }
