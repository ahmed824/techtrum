...
// Static images only (4 items for second section)
const staticImages = [
  // 01 - IT Governance Consulting (compliance, standards, governance)
  "https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=800&h=600&fit=crop",

  // 02 - Digital Transformation Consulting (strategy, business transformation)
  "https://images.unsplash.com/photo-1552664730-d307ca884978?w=800&h=600&fit=crop",

  // 03 - Data Management Consulting (data analytics, databases)
  "https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=800&h=600&fit=crop",

  // 04 - Digital Transformation Solutions & Software (software, no-code platforms)
  "https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800&h=600&fit=crop",
];

export default function SecondServicesScroll({ t: tProp }) {
  ...
  // Build servicesData from translation keys only (no fallback)
  const servicesData = useMemo(() => {
    return Array.from({ length: 4 }, (_, i) => {
      ...
      return {
        ...
        title: t(`services.second.items.${index}.title`),
        description: t(`services.second.items.${index}.description`),
      };
    });
  }, [t, i18n.language]);
  ...
  return (
    <div className="container py-10 md:py-20 min-h-screen px-4">
      ...
      <section
        className="relative mt-20 md:mt-32 lg:mt-0"
        ref={containerRef}
        dir={isRTL ? "rtl" : "ltr"}
      >
        <div
          ...
          <div className="grid lg:grid-cols-[45%_55%] gap-8 lg:gap-16 xl:gap-20">
            ...
            >
              <div className="relative w-full max-w-md lg:max-w-none mx-auto">
                ...
                <div
                  className={`absolute text-4xl sm:text-5xl md:text-7xl font-bold bottom-1 md:bottom-2 text-black ${
                    isRTL
                      ? "-right-8 sm:-right-12 md:-right-20"
                      : "-left-8 sm:-left-12 md:-left-20"
                  }`}
                >
                ...
                <svg
                  className={`absolute -bottom-12 md:-bottom-24 w-40 sm:w-48 md:w-64 h-20 sm:h-24 md:h-32 opacity-10 z-20 ${
                    isRTL
                      ? "right-16 sm:right-20 md:right-28"
                      : "left-16 sm:left-20 md:left-28"
                  }`}
                  viewBox="0 0 256 128"
                  fill="none"
                >
              ...
              {servicesData.map((service, i) => (
                ...
                >
                  ...
                  <div className="pt-1">
                    <div className="flex items-center gap-2 sm:gap-3 md:gap-4 mb-3 md:mb-4 relative">
                      <div
                        className={`absolute top-[70%] -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-sm transition-all duration-300 ${
                          isRTL
                            ? "right-5 sm:right-7 md:right-9"
                            : "left-5 sm:left-7 md:left-9"
                        } ${
                      ...
                      <span
                        className={`relative text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-black ${
                          isRTL ? "pr-1 md:pr-2" : "pl-1 md:pl-2"
                        }`}
                      >
                    ...
                    <p
                      className={`text-black text-sm sm:text-base md:text-lg lg:text-[20px] leading-relaxed max-w-xl ${
                        isRTL
                          ? "pr-12 sm:pr-14 md:pr-16 lg:pr-[72px]"
                          : "pl-12 sm:pl-14 md:pl-16 lg:pl-[72px]"
                      }`}
                    >
                      {service.description}
                    </p>
